...
function s.qecostfilter(c,tp)
	return c:IsRace(RACE_WARRIOR) and c:IsLevelBelow(2) and Cookie3.NoEmFzonefilter(c,tp)
end
function s.QECookiecost(e,tp,eg,ep,ev,re,r,rp,chk)
	return Duel.IsExistingMatchingCard(s.qecostfilter,tp,LOCATION_MZONE,0,1,nil,tp)
end
function s.QECookiecostoperation(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(10060001,1))
	local g=Duel.SelectMatchingCard(tp,s.qecostfilter,tp,LOCATION_MZONE,0,1,1,nil,tp)
...
function s.AndCookieeffcondition(e,tp,eg,ep,ev,re,r,rp)
	...
	return Duel.GetFieldGroupCount(tp,LOCATION_HAND,0)>=5 and bc and Cookie3.NoEmFzonefilter(bc,tp)
end
