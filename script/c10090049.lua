...
function s.low_hp_cookie(c,tp)
	return Cookie3.NoEmFzonefilter(c,tp) and c:GetOverlayCount()<=5
end
function s.AndCookieeffcondition(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(s.low_hp_cookie,tp,LOCATION_MZONE,0,1,nil,tp)
end
function s.AndCookieoperation(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(10060001,1))
	local g=Duel.SelectMatchingCard(tp,s.low_hp_cookie,tp,LOCATION_MZONE,0,0,1,nil,tp)
