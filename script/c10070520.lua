...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.IsExistingTarget(s.filter,tp,LOCATION_MZONE,0,1,e:GetHandler()) end
	Duel.SetChainLimit(aux.FALSE)
end
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	Cookie3.manacost(e,tp,eg,ep,ev,re,r,rp,ATTRIBUTE_DARK,1,1)
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(10060000,6))
	local pcookie=Duel.SelectMatchingCard(tp,s.filter,tp,LOCATION_MZONE,0,1,1,e:GetHandler())
	if Duel.GetAttackTarget()==pcookie:GetFirst() then Duel.NegateAttack() end
	Duel.SendtoGrave(pcookie,REASON_EFFECT)
