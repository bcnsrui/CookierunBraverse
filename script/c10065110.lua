...
function s.lycheedragonfilter(c,tp)
	return c:IsCode(table.unpack(CARD_LYCHEE_DRAGON)) and Cookie3.NoEmFzonefilter(c,tp)
end
function s.Stageoperation(e,tp,eg,ep,ev,re,r,rp)
	local count=Duel.AnnounceNumber(tp,2,1,0)
	if count==0 then return end
	Cookie3.CookieTrashop(e,tp,eg,ep,ev,re,r,rp,count)
	if Duel.IsExistingMatchingCard(s.lycheedragonfilter,tp,LOCATION_MZONE,0,1,nil,tp) then
