...
function s.Trapoperation(e,tp,eg,ep,ev,re,r,rp)
	...
	if ally and ally:GetOverlayCount()>0 then
		local restg=ally:GetOverlayGroup()
		Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(10060001,1))
		local sg=restg:Select(tp,0,1,nil)
		if #sg>0 then Duel.Remove(sg,POS_FACEUP,REASON_EFFECT) end end
end