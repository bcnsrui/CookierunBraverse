...
function s.hp1cookiefilter(c)
	return c:GetOverlayCount()>1 and Cookie3.NoEmFzonefilter(c)
end
function s.Itemcost(e,tp,eg,ep,ev,re,r,rp,chk)
	return Duel.GetMatchingGroupCount(s.hp1cookiefilter,tp,LOCATION_MZONE,0,nil)>=1
end
function s.Itemcostoperation(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(10060000,6))
	local g=Duel.SelectMatchingCard(tp,s.hp1cookiefilter,tp,LOCATION_MZONE,0,1,1,nil)
