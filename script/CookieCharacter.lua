...
function Cookie2.BattleEff(c,attr,colorCount,mixCount)
	...
	e2:SetProperty(EFFECT_FLAG_CANNOT_DISABLE)
	e2:SetOperation(Cookie2.attackreplayeop)
	c:RegisterEffect(e2)
	local e3=Effect.CreateEffect(c)
	e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_CONTINUOUS)
	e3:SetCode(EVENT_ATTACK_ANNOUNCE)
	e3:SetProperty(EFFECT_FLAG_CANNOT_DISABLE)
	e3:SetOperation(Cookie2.attackreplayeop2)
	c:RegisterEffect(e3)
end
function Cookie2.damagecon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler()==Duel.GetAttacker()
end
function Cookie2.attackreplayeop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	...
	e1:SetCondition(Cookie2.battleendcon)
	e1:SetOperation(Cookie2.battleendop)
...
function Cookie2.attackreplayeop2(e,tp,eg,ep,ev,re,r,rp)
	...
	e1:SetCode(EVENT_LEAVE_FIELD)
	e1:SetLabelObject(bc)
	e1:SetCondition(Cookie2.attackreplayecon2)
	e1:SetOperation(Cookie2.attackreplayop2)
	e1:SetReset(RESET_PHASE+PHASE_BATTLE)
	Duel.RegisterEffect(e1,tp)
end
function Cookie2.attackreplayecon2(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetOwner()
	local bc=e:GetLabelObject()
	return Duel.GetCurrentPhase()==PHASE_BATTLE_STEP
	and c and c==Duel.GetAttacker() and bc and bc:IsLocation(LOCATION_MZONE)
