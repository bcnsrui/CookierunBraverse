...
function s.NoEmFzonefilter(c)
	return not (c:IsLocation(LOCATION_EMZONE) or c:GetOverlayCount()<=1)
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return
	Cookie3.manacon(e,tp,eg,ep,ev,re,r,rp,chk,ATTRIBUTE_FIRE,1,1) and
	Duel.IsExistingTarget(s.NoEmFzonefilter,tp,LOCATION_MZONE,0,1,c) end
	Duel.SetChainLimit(aux.FALSE)
end
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	Cookie3.manacost(e,tp,eg,ep,ev,re,r,rp,ATTRIBUTE_FIRE,1,1)
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(id,0))
	local g=Duel.SelectMatchingCard(tp,s.NoEmFzonefilter,tp,LOCATION_MZONE,0,1,1,nil)
